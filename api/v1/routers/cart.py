...
from api.v1.request_models.cart import UpdateCartRequest, AddToCartRequest, RemoveFromCartRequest
...
@post("/add-item", tags=["Cart"], summary="Add item to cart", description="Add a product to the cart.")
async def add_to_cart(
    ...
    return await CartService.add_to_cart(jwt_token, data.product_id, data.quantity, redis)


@post("/remove-item", tags=["Cart"], summary="Remove item from cart", description="Remove a product from the cart.")
...
router = Router(path="/cart", route_handlers=[get_cart, add_to_cart, remove_from_cart, update_cart_item], security=[{"Authentication": ["Bearer"]}])