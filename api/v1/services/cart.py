...
class CartService:
        ...
        try:
            ...
            async with WooCommerceUtils(CONSUMER_KEY, CONSUMER_SECRET, BASE_URL) as woocommerce:
                ...
                if data and data.get("items"):
                    await redis.set(cache_key, data.get("items", []), ttl=300, compress=True)
                    await redis.set(f"cart_token:{user}", data.get("cart_token"), ttl=3600, compress=True)
    ...
    @staticmethod
    async def remove_from_cart(jwt_token: str, product_key: str, redis: AsyncRedisCache):
        try:
            try:
                user = AuthService.decode_jwt_token(jwt_token.replace("Bearer ", "")).get("id")
            except ValueError as e:
